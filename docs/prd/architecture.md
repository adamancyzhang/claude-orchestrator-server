...
```python
def claim_task(instance_id: str) -> Task | None:
    # 1. 获取所有待办任务
    pending = zk.get_children("/tasks/pending")
    sorted_tasks = sort_by_priority_and_fifo(pending, instance_id)
    
    for task_id in sorted_tasks:
        claimed_path = f"/tasks/claimed/{instance_id}-{task_id}"
        try:
            # 2. 原子性创建 claimed 节点
            zk.create(claimed_path, ephemeral=True, 
                      makepath=False)
            # 3. 成功 → 先获取任务数据，再删除 pending 节点
            task_data, _ = zk.get(f"/tasks/pending/{task_id}")
            zk.delete(f"/tasks/pending/{task_id}")
...
关键点：`zk.create(claimed_path)` 是**原子操作** — 同一个路径只能被成功创建一次。

## 会话恢复

当 MCP Server 进程重启时：

1. Kazoo 自动重连 ZK（session timeout 内重连可保留 Ephemeral 节点）。
