...
| 合并冲突 | claude-cli 执行 `git merge --abort` + 抛 `MergeConflictError` |
| claude-cli 决策失败 | MergeValidator 返回 `review_first`（保守） |
| 无 commit（纯文档任务） | MergeValidator 不调用，链正常推进 |

## 10. merge_failed — 任一冲突即链不成功（RC0 新增）

`runMergeValidation(chainId)` 现行为：

```
for commit in chainCommits[chainId]:  # plan→build→verify→review→accept 顺序
  try merge_validator.validate(commit)
  catch err:
    failures.push({ link, sha, branch, message, error })  # 不再吞噬
return failures
```

`close_chain` 分支处理 failures：

- **failures 为空** → `chain_audit.closeChain(chainId, "completed")`，正常关闭
- **failures 非空** →
  1. 对每个失败 record audit 事件 `merge_failure`（payload: sha/branch/message/error）
  2. `chain_audit.closeChain(chainId, "merge_failed", { failures })`
  3. emit `chain_merge_failed` 事件（TUI EVENT LOG 出现红色 `MERGE_FAILED chain <id>: N branch(es) [...] — retry tasks pushed`）
  4. 对每个失败 link 查 `manifest.link_workers[link]` 获取该 link 的 worker，调 `task_queue.push` 创建 retry task：
     - priority = 0（高优）
     - assigned_to = worker.id
     - link = failure.link
     - description 模板：`Merge conflict on branch <branch> at <sha>: <message>.\nError: <error>.\nPull main, resolve conflicts in your worktree, re-commit, and re-run this link.`
  5. emit `chain_closed`（TUI 显示链已结束）
  6. `forgetChain(chainId)`

效果：
- 主分支永远只接受成功合并的 commit，绝不出现"半合并完毕但链标 completed"的诡异状态
- Worker 收到指令性 retry，可在原 worktree 处理冲突；新 commit 由 worker 自身的标准链路推进
- 用户在 TUI 第一时间看到失败，可决定是否手动介入

历史背景：v0.6 早期 `runMergeValidation` 用 `logger.warn` 吞掉所有失败，循环继续；`close_chain` 不论 failures 都写 `status="completed"` → 主线半合并、链标完成、用户无感。详见 R-02。
...
| `completed` | 全部合并成功 | `close_chain` + 失败列表为空 |
| `aborted` | 评估器 reject 或反馈超上限 | `reject` 决策 / `retry_ceiling_exceeded` |
| `merge_failed` | close_chain 命中合并冲突 | 上文 §10 |
