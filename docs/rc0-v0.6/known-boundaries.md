# Known Boundaries — v0.6 RC0

> **文档定位**：v0.6 RC0 显式声明**不保证、不实现**的能力。验收人遇到这些场景时不应当作 RC0 缺陷上报，而应判断是否需要在 v0.7 规划。开发者新增功能前需先确认是否触及这些边界。
...
- **影响**：accept 签字后任何回归发现的问题，需新建 chain 重做；不可复用旧 chain 的 plan/build 产出
- **回避方法**：开新需求时使用新的 chain_id；写新 plan
- **是否计划 v0.7 修复**：候选。引入 `chain_audit.reopenChain(chainId)` API + `/abort`、`/replay`、`/reopen` 三个 TUI 命令
...
- **影响**：Review 想直接退给 Builder、Accept 想直接退给 Builder 的场景受限
- **回避方法**：依靠默认的"一步一步往回退"路径；保证每个中间 link 都能正确反馈
- **是否计划 v0.7 修复**：候选。在 worker prompt 中注入 `{{prev_link_workers_json}}`（manifest.link_workers 的 JSON）让 Worker 能选择跨级目标
...
- **影响**：旧 verify 报告基于过时的 build 跑出，但仍可能影响 review 的输入
- **回避方法**：人工评估是否需要清理 manifest；默认走单步反馈避免此问题
- **是否计划 v0.7 修复**：候选

### 1.4 反馈硬上限不可禁用

- **当前行为**：每条 chain 的总反馈次数 `max_total_retries` 默认 9，可通过 `CO_CHAIN_MAX_RETRIES` 环境变量覆写。超过上限链强制转 `aborted`，无 bypass 选项
- **影响**：极少数复杂任务可能超过 9 次反馈
- **回避方法**：启动时 `export CO_CHAIN_MAX_RETRIES=N` 覆写
- **是否计划 v0.7 修复**：可能调整默认值；不计划提供 disable 选项（防止资源耗尽）

### 1.5 无 deploy 后回归测试钩子

- **当前行为**：MergeValidator 完成 merge 到 main 后无任何 CI/test 触发；merge 是终态，不再有后续校验
- **影响**：merge 后出现的 regression 不会被本系统发现
- **回避方法**：CI 由项目层独立配置
- **是否计划 v0.7 修复**：候选。增加 `merged_chain` 事件与 hook，便于挂接外部 CI

### 1.6 Plan 链节可选（默认仍生成）

- **当前行为**：`ChainDef.tasks.plan` 在 schema 中可为 null，链可跳过 plan 直接从 build 开始；但默认 `worker-decompose.md` 模板始终生成 plan
- **影响**：可定制化跳过 plan，但模板未提供示例
- **回避方法**：定制 `worker-decompose.md` 时显式输出 `"plan": null`
- **是否计划 v0.7 修复**：文档与模板均给出示例
...
- **影响**：Leader 重启窗口期新任务不会被认领
- **回避方法**：监控 Leader 进程并配置自动重启
- **是否计划 v0.7 修复**：不在 v0.7 范围。Leader HA 是 v0.x → v1 的候选

### 2.2 Worker 子进程崩溃重启上限 3 次

- **当前行为**：子进程崩溃父进程自动重启，最多 3 次；超过即认为不可恢复，标 `worker_left`
- **影响**：偶发故障可恢复；持续性故障会下线 Worker
- **是否计划 v0.7 修复**：保留当前行为；可能开放配置项
...
- **是否计划 v0.7 修复**：不计划，扩展即 v0.7 协议破坏
...
- **影响**：非 TypeScript 源码变更不会触发 memory 更新
...
- **影响**：Worker 子进程的 streaming 输出无法在 EVENT LOG 实时观察
- **回避方法**：tail `~/.../tasks/<task_id>/exec-*.log`
- **是否计划 v0.7 修复**：候选
...
- **影响**：脚本化操作需要直接读写 ZK 或封装新 CLI
- **回避方法**：使用 `@co/coordination` 包写自定义脚本
- **是否计划 v0.7 修复**：候选（按需求）

## 6. 应当在 RC0 验收范围内的事项

为了避免误把上述边界当作待修缺陷，明确以下场景必须通过验收：

- 单链 P→B→V→R→A 全程顺利的"快乐路径"
- 任一链节 feedback 单步回退到上一链节
- Worker 子进程崩溃后 task_recovered + 子进程重启（≤3 次）
- close_chain 正常合并到 main
- close_chain 合并冲突路径走 `merge_failed` + builder retry（R-02）
- commit 失败回退为 feedback（R-01）
- 反馈累计超过 max_total_retries 链转 aborted（R-04）
- 不可解析 feedback 静默丢弃（R-05）
- chain_id 重用冲突拒绝（R-06）
- evaluator 三连失败一律 reject（R-03）

详见 `acceptance-checklist.md`。
