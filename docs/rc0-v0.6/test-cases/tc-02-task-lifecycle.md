...
## RC0 新增用例：R-01 commit 失败 → 强制 feedback

### TC-02.RC.1 git commit 真实失败抛 CommitFailedError

落地于 `packages/worker/tests/core/unit/commit-checker.test.ts` "throws CommitFailedError when `git commit` itself fails"。

```typescript
it("should throw CommitFailedError when git commit itself fails", async () => {
  // Given: real on-disk git repo with a pre-commit hook that exits 1
  const repo = initRepo();
  fs.writeFileSync(path.join(repo, "change.txt"), "x");
  installFailingPreCommitHook(repo); // writes .git/hooks/pre-commit, chmod 755

  const checker = makeChecker(repo, "feat: should not land");

  // When + Then:
  await expect(
    checker.check({ link: "build", task_id: asTaskId("t"), task_title: "t", task_description: "d" }),
  ).rejects.toBeInstanceOf(CommitFailedError);

  // HEAD remains at the seed commit — no new commit was made.
  const head = execFileSync("git", ["log", "-1", "--pretty=%s"], { cwd: repo, encoding: "utf-8" }).trim();
  expect(head).toBe("seed");
});
```

### TC-02.RC.2 "无变更"路径仍返回 null

```typescript
it("should return null (not throw) when there are no changes to commit", async () => {
  const repo = initRepo();
  const checker = makeChecker(repo, "no-op");
  const result = await checker.check({ link: "review", task_id: asTaskId("t"), task_title: "t", task_description: "d" });
  expect(result).toBeNull(); // 合法短路，不抛错
});
```

### TC-02.RC.3 Watcher 捕获 CommitFailedError → sendForcedFeedbackReport

集成层断言：commit 失败时 worker 发送 `completion_report` 内容为 `feedback` 决策，且 `feedback_target = self instance_id`。覆盖于 e2e 流程；单元层验证 `commit-checker` 抛错 + `watcher` 路径分支即可。
