...
### TC-03.RC.1 R-04 反馈累计超 max_total_retries → chain aborted

落地于 `packages/leader/tests/core/unit/chain-router.test.ts` "aborts the chain when feedback exceeds max_chain_retries (A5 ceiling)"。

```typescript
it("should abort chain when total_retry_count exceeds max_total_retries", async () => {
  const { router, msg, bus, audit } = setup(
    [makeInstance("tom-01", "Tom", "planner"), makeInstance("jerry-01", "Jerry", "builder")],
    { max_chain_retries: 1 },
  );
  await router.route(chainDefMessage(chainDefJson()));
  await router.route(completionMessage("plan", JSON.stringify({ decision: "activate_next", reason: "ok", next_link: "build" }), asInstanceId("tom-01")));

  // 第一次 feedback：post-increment = 1 <= 1，正常 push
  await router.route(completionMessage("verify", JSON.stringify({ decision: "feedback", reason: "x", feedback_to_worker: "fix #1" }), asInstanceId("lucy-01")));
  expect(msg.sent.find(m => m.task_description === "fix #1")).toBeTruthy();

  // 第二次 feedback：post-increment = 2 > 1，应熔断
  await router.route(completionMessage("verify", JSON.stringify({ decision: "feedback", reason: "x", feedback_to_worker: "fix #2" }), asInstanceId("lucy-01")));
  expect(msg.sent.find(m => m.task_description === "fix #2")).toBeUndefined();
  ...
  expect(bus.emitted.some(e => e.type === "chain_closed")).toBe(true);
});
```

### TC-03.RC.2 R-05 不可解析 feedback target → drop + audit

落地于 `chain-router.test.ts` "drops feedback when neither explicit target nor prior-link worker is resolvable"。

```typescript
it("should drop feedback dispatch when target is unresolvable", async () => {
  const { router, msg, bus } = setup([]); // 无 Tom / Jerry
  await router.route(
    completionMessage("plan", JSON.stringify({
      decision: "feedback",
      reason: "x",
  ...
  // 没有任何 task_dispatch
  expect(msg.sent.find(m => m.task_description === "expand the blueprint")).toBeUndefined();

  // 但 TUI 看到 debug_info
  expect(bus.emitted.some(
    e => e.type === "debug_info" && (e as any).message.includes("no resolvable target")
  )).toBe(true);
});
```

### TC-03.RC.3 R-06 chain_id 重用拒绝

落地于 `packages/leader/tests/core/unit/chain-audit.test.ts` "openChain throws ChainConflictError…"。

```typescript
it("should throw ChainConflictError when reopening a terminal chain", async () => {
  const { audit } = makeAudit();
  const chainId = asChainId("chain-reused");
  await audit.openChain(chainId, { ...meta });
  await audit.closeChain(chainId, "completed");
  await expect(audit.openChain(chainId, { ...meta })).rejects.toBeInstanceOf(ChainConflictError);
});
```

### TC-03.RC.4 R-03 评估器 fallback reject 路由到 closeChain("aborted")

详见 `tc-06-evaluator-fallback.md` Level 2 用例。
