...
## 6.X commit-failure 替代路径（RC0 新增）

正常路径下 Jerry 的 `CommitChecker.check` 返回非空 `CommitResult`（含 sha/message/branch/changed_files/untracked_files）。

异常路径：当 worktree 的 pre-commit hook 拒绝、index 损坏、签名失败等导致 `git commit` 真实失败时（非"无变更"）：

1. `CommitChecker.check` 抛 `CommitFailedError(message, stderr)`。
2. `WorkerWatcher.processTask` 捕获该错：
   - 不调 `recordCommit`（chainCommits 不增项 → close_chain 时 MergeValidator 不会漏走或半走该 link）
   - 不发 `memory_refresh`（无变更可刷新）
   - 调 `sendForcedFeedbackReport(...)` 直接构造 feedback 决策：
     ```json
     {
       "decision": "feedback",
       "reason": "commit failed at build: <stderr 前 200 字符>",
       "feedback_to_worker": "git commit failed for build task <id>. Diagnose with 'git status' / 'git diff' in the worktree, resolve the issue, then re-run.",
       "feedback_target": "jerry-01"
     }
     ```
3. completion_report 走 Leader 标准 feedback 分支：retry task 入 pending，assigned_to=jerry-01，description=feedback_to_worker。
4. retry 同样计入 chain 反馈上限（`CO_CHAIN_MAX_RETRIES`，默认 9）；若耗尽则链转 `aborted`。

后续运行的 retry 任务由 Jerry 自身处理 —— 通常运维介入修复 hook / index 后重新跑，commit 即可正常进入 chainCommits。详见 `core/02` §commit-failure 与 RC0 修复 R-01。
