...
## 9.X close_chain 失败路径：merge_failed（RC0 新增）

`close_chain` 触发 `runMergeValidation` 失败列表非空时：

1. **不**写 `status="completed"`，改写 `closeChain(chainId, "merge_failed", { failures })`，manifest.status 为 `merge_failed`、completed_at 已写。
2. 对每个失败 record `merge_failure` audit 事件，payload 含 `sha / branch / message / error`。
3. emit `chain_merge_failed { chain_id, failures }`，TUI EVENT LOG 出现红色 `MERGE_FAILED chain-pagination-001: 1 branch(es) [co/jerry-01] — retry tasks pushed`。
4. 对每个失败 link 查 manifest.link_workers 获取对应 worker，调 `task_queue.push` 创建 retry task：
   - priority = 0
   - assigned_to = worker.id
   - link = failure.link（如 build → jerry-01）
   - description 模板：`Merge conflict on branch co/jerry-01 at <sha>: <msg>.\nError: <err>.\nPull main, resolve conflicts in your worktree, re-commit, and re-run this link.`
5. emit `chain_closed`（TUI 链已结束，但状态是 merge_failed 而非 completed）。
6. `forgetChain(chainId)`。

Worker 收到 retry 后在原 worktree 处理冲突（pull main → 解决冲突 → 重新 commit），新 commit 由 worker 自身的标准链路推进。如果运维决定不修复，链就此终止；用户在 TUI 第一时间看到失败，与 v0.6 早期的"无声半合并"对比是关键区别。详见 `core/04` §merge_failed、RC0 修复 R-02。

## 9.Y 反馈上限熔断：CO_CHAIN_MAX_RETRIES（RC0 新增）

链的 `total_retry_count` 每次反馈 +1（无论是 Verify→Build、Review→Verify、Accept→Review，还是任何跨级 feedback）。

超过 `max_total_retries`（默认 9，环境变量 `CO_CHAIN_MAX_RETRIES` 覆写）时：

- ChainRouter 不 push 新 retry task
- closeChain(chainId, "aborted", { reason: "retry_ceiling_exceeded", total_retry_count, max_total_retries })
- emit `debug_info`：`chain <id> aborted: retry ceiling N exceeded`
- emit `chain_closed`

效果：极端反复反馈的链路被强制终止，不会无限消耗 ZK 节点和 cache 磁盘。详见 `dd/error-and-recovery.md` §retry-ceiling 与 RC0 修复 R-04。
