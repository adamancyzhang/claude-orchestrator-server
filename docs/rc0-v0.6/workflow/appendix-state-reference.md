...
task_created / task_claimed / task_completed / task_blocked / task_failed /
task_recovered / task_dependency_resolved /
message_sent / message_received / message_processed /
chain_activated / chain_closed / chain_merge_failed (RC0 新增) /
debug_info / stream_chunk
```

ChainRouter emit：
- `chain_activated`（handleTaskDefinitions 首次入队 5 个 task 时）
- `chain_closed`（reject / close_chain / retry_ceiling_exceeded / merge_failed 任一情况）
- `chain_merge_failed`（close_chain 内合并冲突时，与 chain_closed 配对出现）
- `debug_info`（feedback_unresolved / chain_id_conflict / retry_ceiling_exceeded 等用户可见告警）

链路推进的细粒度事件（task_dispatch / feedback_sent / feedback_unresolved / merge_failure / chain_id_conflict / retry_ceiling_exceeded / completion_report 等）记录在 `chains/<chain_id>/audit.jsonl`，需要时可按 ts 时间戳线性回放。

## ChainManifest schema（RC0 字段一览）
...
```json
{
  ...
  "completed_at": null,
  "status": "running",            // running | completed | failed | aborted | merge_failed (RC0 新增)
  "leader_id": "leader-01",
  "leader_name": "Leader",
  "requirement_path": "...",
  "link_tasks":   { "plan": "...", "build": "...", "verify": "...", "review": "...", "accept": "..." },
  "link_workers": { "plan": "tom-01", "build": "jerry-01", "verify": "lucy-01", "review": "mia-01", "accept": "leo-01" },
  "total_retry_count": 0,         // RC0 新增；ChainAudit.incrementRetry 递增
  "max_total_retries": 9          // RC0 新增；默认 9，CO_CHAIN_MAX_RETRIES 覆写
}
```

## audit.jsonl 事件类型一览（RC0）

| 事件 | 触发 | payload 关键字段 |
|------|------|-----------------|
| `chain_opened` | ChainAudit.openChain | created_at / leader_id / max_total_retries |
| `requirement_received` | handleTaskDefinitions 首次保存 requirement.md | requirement_path |
| `task_dispatch` | 每次派发任务给 worker | link / worker_id / task_id |
| `completion_report` | 每次收到 worker 完成报告 | decision |
| `feedback_sent` | dispatchFeedbackAsRetry 成功 push retry task | feedback_to_worker / retry_count / target_link |
| `feedback_unresolved` | RC0 新增；feedback target 不可解析 | feedback_to_worker / explicit_target |
| `chain_id_conflict` | RC0 新增；重用已关闭 chain_id | existing_status / existing_completed_at |
| `merge_failure` | RC0 新增；close_chain 时某 commit 合并失败 | sha / branch / message / error |
| `retry_ceiling_exceeded` | RC0 新增；feedback 累计超 max_total_retries | total_retry_count / max_total_retries |
| `chain_closed` | closeChain 写入终态 | status / extra |
