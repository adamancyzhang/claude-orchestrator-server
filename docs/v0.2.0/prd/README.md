...
| `server_status` | — | `str` | 健康检查，返回 ZK 连接状态和服务端口 |
...
| Prompt | 参数 | 说明 |
|--------|------|------|
| `orchestrate-task` | `goal: str, context?: str` | 帮助将目标拆解为可分配的任务，结合团队状态给出建议 |
| `team-status` | — | 生成当前所有实例和任务的状态报告 |
...
| 组件 | 选型 | 理由 |
|------|------|------|
| 语言 | TypeScript 5.6+ / Node.js 18+ | 纯 npm 分发，与 Claude Code 生态统一 |
| MCP 框架 | `@modelcontextprotocol/sdk` | Anthropic 官方 TypeScript SDK |
| HTTP 框架 | Express | 轻量 HTTP 服务，Streamable HTTP 传输 |
| 数据校验 | Zod | 运行时类型校验，与 MCP SDK 深度集成 |
| CLI 框架 | Commander | Node.js 标准 CLI 框架 |
| 传输协议 | Streamable HTTP (SSE) | 多客户端长连接，MCP 协议通知推送 |
| 分布式协调 | ZooKeeper 3.8+ | 服务发现 + Watch + 顺序节点 |
| ZK 客户端 | `node-zookeeper-client` | Node.js 最成熟的 ZK 客户端库 |
...
│   ├── server.ts                  # MCP 服务端 — 工具/资源/提示注册
│   ├── config.ts                  # 配置管理
│   ├── cli/
│   │   └── commands.ts            # CLI 子命令实现
│   ├── zk/
│   │   ├── client.ts              # ZooKeeper 连接管理与工具函数
│   │   ├── paths.ts               # ZK 路径常量
│   │   └── watcher.ts             # ZK Watch 管理器
│   ├── modules/
│   │   ├── registry.ts            # 实例注册表
│   │   ├── task-queue.ts          # 任务队列
│   │   ├── message-router.ts      # 消息路由 + Resource Subscription + wait_for_message
│   │   └── context-store.ts       # 共享键值存储
│   ├── models/
│   │   └── schemas.ts             # Zod 模式与推断类型
│   └── utils/
│       └── output.ts              # CLI 输出格式化
├── bin/
