...
需求进入
    ...
    ▼
┌─────────┐    ┌──────────┐    ┌─────────┐    ┌──────────┐    ┌──────────┐
│  Plan   │ ─▶ │ Execute  │ ─▶ │ Verify  │ ─▶ │  Review  │ ─▶ │  Accept  │ ─▶ 闭环
...
... Plan → Execute → Verify → Review → Accept → Explore
                                          ...
                                          ┌────────┴─────────┐
                                          │                  │
                                     spawn_chain         close_chain
                                          │                  │
                                          ▼                  ▼
                                    起新需求            循环终止
                                    新 chain_id            +
                                    新一轮闭环         MergeValidator
```
...
| 环节 | 主要产出 | 进入下一环节的条件 |
|------|---------|------------------|
| Plan | 蓝图文档、任务规格 | 蓝图清晰，Executor 可直接动手 |
...
| decision | 触发的下一动作 | 合法 link |
|---------|---------------|----------|
| `activate_next` | 派发下一 link 的任务 | plan / execute / verify / review；--magic 下 accept 也合法（next = explore） |
...
- `merge conflict` → 链终态 `merge_failed`，TUI EVENT LOG 出现红色 `MERGE_FAILED chain <id>: N branch(es) ...`，Executor 收件箱出现 conflict resolve 任务
