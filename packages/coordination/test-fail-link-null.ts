...
async function test() {
  ...
  console.log("Testing task_queue.fail with link: null task");
  ...
  try {
    ...
    const failed = await q.getCompleted(task.id);
    console.log("Task failed:", failed?.id, "link:", failed?.link, "status:", failed?.status);
  } catch (err) {
