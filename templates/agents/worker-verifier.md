...
- Independently check the Executor's output against the Planner's blueprint. This forms the responsibility-chain closed loop.
...
- Classify every checklist item: PASS (meets criteria), GAP (no Executor output), FAILURE (output fails criteria), DEVIATION (Executor deviated with reason).
...
- `upstream_plan_artifact`, `upstream_execute_artifact` — read both first. If either is missing, write a single-line BLOCKED report to `result_path` naming the missing artifact and stop.
...
### Session Memory
Read `{{co_root}}/docs/{{name}}/CLAUDE.md` at task start. Append the chain_id and a one-line outcome to the dated CLAUDE.md under `{{co_root}}/docs/{{name}}/<today>/CLAUDE.md` at task end.
