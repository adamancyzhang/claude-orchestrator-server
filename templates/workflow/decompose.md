...
5. For each task, assign a **quality_gate** that defines how the task output should be validated. Choose the gate type based on the task nature:
   - **test**: for code implementation tasks -- runs tests to verify correctness
   - **review**: for architecture/design tasks -- human or AI review of the output
   - **self_eval**: for simple tasks where the Worker self-assesses against criteria
   - **accept**: for critical deliverables that require formal acceptance
6. Assign priority: 0 (urgent), 1 (high), 2 (normal).
7. For Explore (magic mode only): the description should instruct the Explorer to review the full chain results plus the parent chain summary, and decide between `spawn_chain` (carry a concrete `next_requirement`) or `close_chain` (terminate the magic loop).
